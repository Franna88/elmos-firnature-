...
class AnalyticsService extends ChangeNotifier {
  ...
  Future<void> _saveLocalEvents() async {
    try {
      ...
      await _prefs!.setString('local_analytics_events', jsonEncode(_localEvents));
    } catch (e) {
      if (kDebugMode) {
  ...
  Future<void> _generateAnalyticsData() async {
    ...
    try {
      ...
      for (var doc in sopsSnapshot.docs) {
        ...
        if (updatedAt != null && updatedAt.isAfter(firstDayOfMonth) && 
  ...
  Future<void> recordActivity(ActivityType type, {String? sopId, String? sopTitle}) async {
    final now = DateTime.now();
    final userId = _auth?.currentUser?.uid;
    final userName = _authService.userName ?? 'User';
